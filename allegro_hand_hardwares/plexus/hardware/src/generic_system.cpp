...
#include "allegro_hand_plexus_hardware/generic_system.hpp"
...
CallbackReturn GenericSystem::on_init(const hardware_interface::HardwareInfo& info) {
  ...
  auto populate_non_standard_interfaces = [this](auto interface_list, auto& non_standard_interfaces) {
    for (const auto& interface : interface_list) {
      ...
      if (std::find(standard_interfaces_.begin(), standard_interfaces_.end(), interface.name) == standard_interfaces_.end()) {
        if (std::find(non_standard_interfaces.begin(), non_standard_interfaces.end(), interface.name) == non_standard_interfaces.end()) {
  ...
  if (it != info_.hardware_parameters.end()) {
    ...
    if (it != info_.hardware_parameters.end()) {
      use_mock_sensor_command_interfaces_ = hardware_interface::parse_bool(it->second);
      RCUTILS_LOG_WARN_NAMED("mock_generic_system", "Parameter 'fake_sensor_commands' has been deprecated from usage. Use"
  ...
  if (it != info_.hardware_parameters.end()) {
    ...
    if (it != info_.hardware_parameters.end()) {
      use_mock_gpio_command_interfaces_ = hardware_interface::parse_bool(it->second);
      RCUTILS_LOG_WARN_NAMED("mock_generic_system", "Parameter 'fake_gpio_commands' has been deprecated from usage. Use"
  ...
  for (auto i = 0u; i < info_.joints.size(); ++i) {
    ...
    if (joint.parameters.find("mimic") != joint.parameters.cend()) {
      ...
      mimic_joint.mimicked_joint_index = static_cast<size_t>(std::distance(info_.joints.begin(), mimicked_joint_it));
  ...
  if (!custom_interface_with_following_offset_.empty()) {
    auto if_it = std::find(other_interfaces_.begin(), other_interfaces_.end(), custom_interface_with_following_offset_);
    if (if_it != other_interfaces_.end()) {
      index_custom_interface_with_following_offset_ = static_cast<size_t>(std::distance(other_interfaces_.begin(), if_it));
      RCUTILS_LOG_INFO_NAMED("mock_generic_system", "Custom interface with following offset '%s' found at index: %zu.",
                             custom_interface_with_following_offset_.c_str(), index_custom_interface_with_following_offset_);
    } else {
      RCUTILS_LOG_WARN_NAMED("mock_generic_system",
  ...
  for (const auto& sensor : info_.sensors) {
    for (const auto& interface : sensor.state_interfaces) {
      if (std::find(sensor_interfaces_.begin(), sensor_interfaces_.end(), interface.name) == sensor_interfaces_.end()) {
        sensor_interfaces_.emplace_back(interface.name);
      }
    }
  }
  initialize_storage_vectors(sensor_mock_commands_, sensor_states_, sensor_interfaces_, info_.sensors);
...
std::vector<hardware_interface::StateInterface> GenericSystem::export_state_interfaces() {
    ...
    for (const auto& interface : joint.state_interfaces) {
      ...
      if (!get_interface(joint.name, standard_interfaces_, interface.name, i, joint_states_, state_interfaces)) {
        if (!get_interface(joint.name, other_interfaces_, interface.name, i, other_states_, state_interfaces)) {
          throw std::runtime_error("Interface is not found in the standard nor other list. "
  ...
  if (!populate_interfaces(info_.sensors, sensor_interfaces_, sensor_states_, state_interfaces, true)) {
    throw std::runtime_error("Interface is not found in the standard nor other list. This should never happen!");
  };
...
std::vector<hardware_interface::CommandInterface> GenericSystem::export_command_interfaces() {
  ...
  if (use_mock_sensor_command_interfaces_) {
    if (!populate_interfaces(info_.sensors, sensor_interfaces_, sensor_mock_commands_, command_interfaces, true)) {
      throw std::runtime_error("Interface is not found in the standard nor other list. This should never happen!");
    }
  }
  ...
  if (use_mock_gpio_command_interfaces_) {
    if (!populate_interfaces(info_.gpios, gpio_interfaces_, gpio_mock_commands_, command_interfaces, true)) {
      throw std::runtime_error("Interface is not found in the gpio list. This should never happen!");
    }
  }
  // GPIO command interfaces (real command interfaces)
  else {
    if (!populate_interfaces(info_.gpios, gpio_interfaces_, gpio_commands_, command_interfaces, false)) {
      throw std::runtime_error("Interface is not found in the gpio list. This should never happen!");
    }
  }
...
return_type GenericSystem::prepare_command_mode_switch(const std::vector<std::string>& start_interfaces,
  ...
  for (const auto& key : start_interfaces) {
    ...
    auto joint_it_found = std::find_if(info_.joints.begin(), info_.joints.end(),
    ...
    if (joint_it_found != info_.joints.end()) {
      ...
      RCUTILS_LOG_DEBUG_NAMED("mock_generic_system", "Got interface '%s' that is not joint - nothing to do!", key.c_str());
    }
  }
  ...
  for (size_t i = 0; i < info_.joints.size(); ++i) {
    ...
    if (joint_found_in_x_requests_[i] == FOUND_ONCE_FLAG) {
      RCUTILS_LOG_ERROR_NAMED("mock_generic_system", "Joint '%s' has to have '%s', '%s', or '%s' interface!", info_.joints[i].name.c_str(),
    ...
    if (joint_found_in_x_requests_[i] > (FOUND_ONCE_FLAG + 1)) {
      RCUTILS_LOG_ERROR_NAMED("mock_generic_system",
...
return_type GenericSystem::perform_command_mode_switch(const std::vector<std::string>& start_interfaces,
                                                       const std::vector<std::string>& /*stop_interfaces*/) {
  if (!calculate_dynamics_) {
...
return_type GenericSystem::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& period) {
  if (command_propagation_disabled_) {
    RCUTILS_LOG_WARN_NAMED("mock_generic_system", "Command propagation is disabled - no values will be returned!");
  ...
  for (size_t j = 0; j < joint_states_[POSITION_INTERFACE_INDEX].size(); ++j) {
    if (calculate_dynamics_) {
      ...
      case ACCELERATION_INTERFACE_INDEX: {
        ...
        joint_states_[VELOCITY_INTERFACE_INDEX][j] += joint_states_[ACCELERATION_INTERFACE_INDEX][j] * period.seconds();

        if (!std::isnan(joint_commands_[ACCELERATION_INTERFACE_INDEX][j])) {
          joint_states_[ACCELERATION_INTERFACE_INDEX][j] = joint_commands_[ACCELERATION_INTERFACE_INDEX][j];
        }
        break;
      }
      case VELOCITY_INTERFACE_INDEX: {
        ...
        if (!std::isnan(joint_commands_[VELOCITY_INTERFACE_INDEX][j])) {
          ...
          joint_states_[ACCELERATION_INTERFACE_INDEX][j] = (joint_states_[VELOCITY_INTERFACE_INDEX][j] - old_velocity) / period.seconds();
        }
        break;
      }
      case POSITION_INTERFACE_INDEX: {
        if (!std::isnan(joint_commands_[POSITION_INTERFACE_INDEX][j])) {
          ...
          joint_states_[VELOCITY_INTERFACE_INDEX][j] = (joint_states_[POSITION_INTERFACE_INDEX][j] - old_position) / period.seconds();
  ...
  for (const auto& mimic_joint : mimic_joints_) {
    for (auto i = 0u; i < joint_states_.size(); ++i) {
      joint_states_[i][mimic_joint.joint_index] = mimic_joint.multiplier * joint_states_[i][mimic_joint.mimicked_joint_index];
    }
  }

  for (size_t i = 0; i < other_states_.size(); ++i) {
    for (size_t j = 0; j < other_states_[i].size(); ++j) {
      if (i == index_custom_interface_with_following_offset_ && !std::isnan(joint_commands_[POSITION_INTERFACE_INDEX][j])) {
        other_states_[i][j] = joint_commands_[POSITION_INTERFACE_INDEX][j] + position_state_following_offset_;
...
bool GenericSystem::get_interface(const std::string& name, const std::vector<std::string>& interface_list,
                                  const std::string& interface_name, const size_t vector_index, std::vector<std::vector<double>>& values,
...
void GenericSystem::initialize_storage_vectors(std::vector<std::vector<double>>& commands, std::vector<std::vector<double>>& states,
  ...
  if (print_hint) {
    RCUTILS_LOG_WARN_ONCE_NAMED("mock_generic_system",
...
bool GenericSystem::populate_interfaces(const std::vector<hardware_interface::ComponentInfo>& components,
  ...
  for (auto i = 0u; i < components.size(); i++) {
    ...
    const auto interfaces = (using_state_interfaces) ? component.state_interfaces : component.command_interfaces;
    for (const auto& interface : interfaces) {
      if (!get_interface(component.name, interface_names, interface.name, i, storage, target_interfaces)) {
        return false;
      }
    }
  }
...
PLUGINLIB_EXPORT_CLASS(allegro_hand_plexus_hardware::GenericSystem, hardware_interface::SystemInterface)
